 ...
 *  Rev. 0.3: Expansion from 8 to 14 kernals, no bootloader, no RX, TX (TMO)
 *  Rev. 0.3_basic: Expansion from 8 to 14 kernals, no bootloader, no RX, TX (TMO), Basic in first bank
 */
...
//TMO: const int  NumKernal = 8;            //This is the highest valid Kernal number
const int  NumKernal = 14;            //This is the highest valid Kernal number
...
const int col5 = 0;        //TNO: input, column signal, active low
const int col6 = 1;        //TMO: input, column signal, active low
...
//TMO: const int Signalling = 10; // Pin for signaling (LED or buzzer with tone generator), pin header J4, pin 6
const int KSW_A16 = 10; // TMO: output, A17 to kernal adaptor/switch
...
//TMO volatile byte ColData = 0x1F;  // column data, read from PORTC
volatile byte ColData = 0x7F;  // column data, read from PORTC and PORTD
...
void setup() {
  ...
  pinMode( col5, INPUT_PULLUP);  //TMO: pullup required for MOS logic
  pinMode( col6, INPUT_PULLUP);  //TMO: pullup required for MOS logic
...
//TMO:  digitalWrite( Signalling, sig_idle ); // Signalling pin6, J4
//TMO:  pinMode( Signalling, OUTPUT);
...
//TMO:  if ((KNumber < 1) || (KNumber > 8-KOffset)) { // if out of range, the first kernal is selected.
  if ((KNumber < 1) || (KNumber > NumKernal)) { // if out of range, the first kernal is selected.
    KNumber = 1;
  ...
  //TMO: For long boards, it should be between 0 and 13,
  //TMO: for short boards, it should be 1 to 14, because the first 8k are occupied by the BASIC ROM.
  ...
  pinMode( KSW_A16, OUTPUT );     // TMO: make it an output   
...
/*TMO:  Serial.begin(9600);  // start serial interface
  /* output status etc. */
...
void SetAddressBits( byte addr ) {
  ...
  if ((addr & 8) == 0) {          //TMO: A16
    digitalWrite(KSW_A16, LOW);   //TMO
  }                               //TMO
  else {                          //TMO
    digitalWrite(KSW_A16, HIGH);  //TMO 
  }                               //TMO
}

void ResetC64() {
//TMO:  digitalWrite( Signalling, !sig_idle );                     // Signalling active
  ...
  countup = 0;
//TMO:  digitalWrite( Signalling, sig_idle );                      // Signalling inactive
}

void SwitchKernal() {
  digitalWrite( C64RESET, HIGH );                            // issue a reset pulse for the C64
  SetAddressBits( KAddr );                                   // set the address bits A16..A13
//TMO:  Serial.print("Change to Kernal number: ");                 // send the status to serial
//TMO:  Serial.println( KNumber );
...
//TMO:    ColData = PINC & 0x1F;               // read the columns
     ColData = (PINC & 0x1F) | ((PIND & 0x03) << 5);       //TMO: read the columns, PIND bitshift um 5 nach links
     scan_found = true;                   // any activity here indicates a working kernal
     NumOdd = 1;                          // row0 has the odd numbers
//TMO:     if ((ColData != 0x1F) && (ColData !=0)) {               // only set the flag, when a number is pressed 
     if ((ColData != 0x7F) && (ColData !=0)) {               //TMO: only set the flag, when a number is pressed
       NumPressed = 1;
     }  
  }
}
...
//TMO:     columns = PINC & 0x1F;                              // read the column data
     columns = (PINC & 0x1F) | ((PIND & 0x03) << 5);     //TMO: read the column data
//TMO:    if ((columns != 0x1F)) {                            // activity found?
    if ((columns != 0x7F)) {                            //TMO: activity found?
      scan_found = true;                                // set flag
//TMO:      Serial.println( "Keyboard scan detected."); 
    }
    else {
...
//TMO:        Serial.println( "No keyboard scan detected.");  // the count down is elapsed!
  ...
  if (digitalRead( nRESTORE ) == HIGH) {
     if (RestoreKey == 1) {                // now no RESTORE, but last status was RESTORE => Restore key released 
        if (countup > EXROM_Count) {       // is the limit for EXROM reset reached?
          exrom_mode = 1;
//TMO:          Serial.print( "EXROM mode: ");
        }
        if (countup > Restore_Count) {
//TMO:          Serial.println("Restore key => Reseting the C64");
          ResetC64();                      // RESET the C64
          exrom_mode = 0;
        }
     }
...
//TMO:      digitalWrite( Signalling, sig_idle );
  ...
  */
  if (NumPressed == 1) {                     // if a number key is pressed: process the data
    ...
    else if ((ColData & B00100000) == 0) {   // column 5: "+"        //TMO key "+" (11)
      key = 11;                                                      //TMO
    }                                                                //TMO
    else if ((ColData & B01000000) == 0) {   // column 6: "Pfund"    //TMO key "Pfund" (13)
      key = 13;                                                      //TMO
    }                                                                //TMO
    /*
...
//TMO:    if (key == 10) {                         // in case it is a "0" 
//TMO:      key = 0;
//TMO:    }

//TMO:    Serial.print("Key ");
//TMO:    Serial.print(key);
//TMO:    Serial.println(" detected.");
...
//TMO:       KAddr = KNumber - 1 + KOffset;                             // Calculate the address bits. 
                                                                  // For long boards, it should be between 0 and 7,
                                                                  // for short boards, it should be 1 to 7, because the first 8k are occupied by the BASIC ROM. 
       if (KOffset == 0)                                          //TMO
          KAddr = KNumber - 1;                                    //TMO
...
//TMO:KAddr = KNumber - 1 + KOffset;
      ...
      EEPROM.write(EEPROMAddr+1, inactive);                       // write it to EEPROM
//TMO:      Serial.println( "Kernal 1 and scan timeout inactive." );    // send to serial
      SwitchKernal();                                             // Switch Kernal Number and reset C64
    }
    else {
